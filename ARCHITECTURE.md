...
| Game Controller | `src/game/` | Per-life Game instance, state-machine dispatch, collision/score logic | `game.js`, `gameState.js`, `soundManager.js`, `helpers.js` |
...
| Input | Polled per-frame via `p5.keyIsDown(<keyCode>)` for movement; transient via reassigning `p5.keyPressed` for shooting and screen transitions | `src/game/elements/ship.js` (`shoot()`), each screen class (own keyPressed handler calling a `state` transition method) |
